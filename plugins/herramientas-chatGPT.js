...
if (!text) throw `insira algum texto de solicitação ou pedido para usar o chatgpt.\n\n❏ alguns exemplos:\n-> ${usedPrefix + command} me recomende um dos 10 melhores filmes de ação.\n-> ${usedPrefix + command} me explique o significado de verbo.`     
try {
conn.sendPresenceUpdate('composing', m.chat)  
let chgptdb = global.chatgpt.data.users[m.sender];
chgptdb.push({ role: 'user', content: text });
const config = { method: 'post', url: 'https://api.openai.com/v1/chat/completions', headers: { 'Content-Type': 'application/json', 'Authorization': 'Bearer ' + global.openai_key }, data: JSON.stringify({ 'model': 'gpt-3.5-turbo', 'messages': [{ role: 'system', content: 'Você atuará como um WhatsApp Bot criado por Anuudek. Você será o botAnuudek e deverá escrever tudo em minúsculo, pois é a sua estética' }, ...chgptdb ]})}
...
let akuariapiresult1 = await translate(`${akuariapijson1.respon}`, { to: 'pt', autoCorrect: true })
...
let hasill22_result = await translate(`${hasill22.result}`, { to: 'pt', autoCorrect: true })  
...
let akuariapiresult2 = await translate(`${akuariapijson2.respon}`, { to: 'pt', autoCorrect: true })
m.reply(akuariapiresult2.text.trim())    
} catch {    
try {   
let syms1 = `Você atuará como um WhatsApp Bot criado por Anuudek. Você será o botAnuudek e deverá escrever tudo em minúsculo, pois é a sua estética`  
...
let fgjson1_result = await translate(`${fgjson1.result}`, { to: 'pt', autoCorrect: true })  
